...
(function () {
    ...
    const self = document.querySelector("body > #search-hospital");
    ...
    self.addEventListener("hey", function () {
        ...
        current_data = Cache.Get("hospital");
        filter();
    });
    ...
    function jump() {
        for (var i = 0; i < current_data_filter.length; i++) {
            ...
            div.style.backgroundImage = "url('" + Api.Storage(current_data_filter[i].image.id, current_data_filter[i].image.extension) + "')";
            div.querySelector(":scope > header").innerHTML = current_data_filter[i].name.en;
            div.querySelector(":scope > a").href = "#l=hospital&id=" + current_data_filter[i].id;
