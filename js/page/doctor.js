...
(function () {
    ...
    self.querySelector(":scope > header > main > header > div.favorite").addEventListener("click", function () {
        if (!this.classList.contains("favorited")) {
            ...
            user.favorite_doctor_list = user.favorite_doctor_list || [];
            if (user.favorite_doctor_list.indexOf(doctor_id) >= 0) {
                user.favorite_doctor_list.splice(user.favorite_doctor_list.indexOf(doctor_id), 1);
            }
            Api.Core("user", "update", user, function () {
                Auth.Login(user.token, user);
                test_favorited();
    ...
    function test_favorited() {
        if (Auth.Test()) {
            ...
            if (user.favorite_doctor_list.indexOf(Param.Get("id")) > -1) {
                ...
                self.querySelectorAll(":scope > header > main > header > div > svg.on").removeClass("on");
                self.querySelectorAll(":scope > header > main > header > div > svg.favorited").addClass("on");
            } else {
                ...
                self.querySelectorAll(":scope > header > main > header > div > svg.on").removeClass("on");
                self.querySelectorAll(":scope > header > main > header > div > svg.favorite").addClass("on");
            }
        } else {
        ...
        Api.Core("doctor", "single", {
            ...
            Api.Core("comment", "filter", {
                ...
                for (var i = 0; i < comments.length; i++) {
                    holder.innerHTML = self.querySelector(":scope > footer > main > main > template[data-star='" + comments[i].star + "']").innerHTML;
